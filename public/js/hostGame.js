...
socket.on('GameOver', function(data) {
    ...
    // document.getElementById('question').innerHTML = "Quiz Finish!!";
    document.getElementById('finish').style.display = "block";
    document.getElementById('fileToExport').style.display = "block";
    ...
    let winner = document.getElementById('winner')
        // winner.innerHTML += `${data['num'+1].name} Score: ${data['num'+1].score} <br/>`
    winner.innerHTML = "<tr><th>No.</th><th>Nim </th><th>Name</th><th>Score </th></tr>"
    for (var i = 0; i < data.length; i++) {
        console.log(data)
        winner.innerHTML += `<tr> <td>${i+1}.</td><td>${data[i].nim}</td><td>${data[i].name}</td><td>${data[i].score}</td></tr>`
    }
