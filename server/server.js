    ...
    socket.on('player-join', (params) => {
        ...
        for (var i = 0; i < games.games.length; i++) {
            ...
            if (params.pin == games.games[i].pin) {
                ...
                players.addPlayer(hostId, socket.id, params.name + "-" + params.nim, { score: 0, answer: 0 }); //add player to game
            ...
            dbo.collection("kahootGames").find(query).toArray(function (err, res) {
                ...
                if (res[0].questions.length >= game.gameData.question) {
                    ...
                    for (var i = 0; i < playersInGame.length; i++) {
                        // dataScore['num' + (i + 1).toString()] = { name: `${playersInGame[i].name}`, score: `${playersInGame[i].gameData.score}` }
                        let playerScore = { name: `${playersInGame[i].name.split("-")[0]}`,nim: `${playersInGame[i].name.split("-")[1]}`, score: `${playersInGame[i].gameData.score}` }
                        dataScore.push(playerScore)
                    }
                    io.to(game.pin).emit('GameOver', dataScore);
